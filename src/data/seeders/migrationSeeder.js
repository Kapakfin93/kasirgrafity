...
const NEW_PILLARS = [
    {
        id: 'LARGE_FORMAT',
        name: 'Large Format (Outdoor & Textile)',
        logic_type: 'HYBRID',
        sort_order: 1,
        is_active: 1
    },
...
const LEGACY_OFFICE_PRODUCTS = [
    'o1', 'o2', 'o3', 'o4', // Stempel, Nota NCR (simple)
    'c1', 'c2'              // Kalender (simple)
];
...
async function migrateProducts() {
    ...
    for (const cat of allCategories) {
        ...
        // Name-based matching
        if (name.includes('banner') || name.includes('spanduk') ||
            name.includes('poster') || name.includes('textile') || name.includes('dtf')) {
            targetPillar = 'LARGE_FORMAT';
        } else if (name.includes('a3') || name.includes('digital') ||
            name.includes('promo') || name.includes('dokumen') || name.includes('stiker')) {
            targetPillar = 'DIGITAL_A3_PRO';
        } else if (name.includes('office') || name.includes('calendar') ||
            name.includes('kalender') || name.includes('nota') || name.includes('ncr')) {
            targetPillar = 'STATIONERY_OFFICE';
        } else if (name.includes('merch') || name.includes('apparel') ||
            name.includes('display') || name.includes('jersey') || name.includes('kaos')) {
            targetPillar = 'MERCH_APPAREL';
        }

        if (targetPillar) {
            categoryMapping.set(cat.id, targetPillar);
            console.log(`    📌 "${cat.name}" (${cat.id.substring(0, 8)}...) → ${targetPillar}`);
        }
    }
    ...
    for (const product of allProducts) {
        try {
            ...
            } else if (['LARGE_FORMAT', 'DIGITAL_A3_PRO', 'STATIONERY_OFFICE', 'MERCH_APPAREL'].includes(product.categoryId)) {
                // Already in a new pillar (e.g., pilot product)
                console.log(`  ✅ [ALREADY MIGRATED] ${product.name} (${product.categoryId})`);
                migrationStats.skipped.count++;
            } else {
                // Orphaned product (unknown category)
                console.warn(`  ⚠️  [ORPHAN] ${product.name} (categoryId: ${product.categoryId})`);
                migrationStats.skipped.count++;
            }
    ...
    // Summary
    console.log('\n  📊 Migration Summary:');
    console.log(`    LARGE_FORMAT: ${migrationStats.LARGE_FORMAT.moved} products`);
    console.log(`    DIGITAL_A3_PRO: ${migrationStats.DIGITAL_A3_PRO.moved} products`);
    console.log(`    STATIONERY_OFFICE: ${migrationStats.STATIONERY_OFFICE.moved} products`);
    console.log(`    MERCH_APPAREL: ${migrationStats.MERCH_APPAREL.moved} products`);
...
async function archiveOldCategories() {
    ...
    const SAFE_IDS = ['LARGE_FORMAT', 'DIGITAL_A3_PRO', 'STATIONERY_OFFICE', 'MERCH_APPAREL'];
    ...
    for (const category of allCategories) {
        try {
            ...
            if (!SAFE_IDS.includes(category.id)) {
                ...
                console.log(`  🗑️  [CLEANUP] Archived: ${category.name} (ID: ${category.id})`);
                archiveResults.push({
                    id: category.id,
                    name: category.name,
                    action: 'ARCHIVED'
...
async function verifyMigration() {
    ...
    for (const pillar of NEW_PILLARS) {
        const count = await db.products
            .where('categoryId').equals(pillar.id)
...
export async function runMigration() {
    ...
    try {
        ...
        if (categoryCount === 0) {
            console.log('📦 STEP 0: Database empty - seeding initial data from MASTER_DATA...');
            await seedMasterData();
            console.log('  ✅ Initial data seeded\n');
        } else {
            console.log(`📦 STEP 0: Database has ${categoryCount} categories - skipping initial seed\n`);
        }
