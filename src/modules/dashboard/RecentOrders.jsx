...
export function RecentOrders({ orders }) {
  ...
  const getProductPreview = (order) => {
    ...
    const productName =
      firstItem.productName || firstItem.description?.split(" ")[0] || "Item";
  ...
  return (
    <div className="recent-orders-list">
      {orders.map((order) => {
        const statusConfig =
          ORDER_STATUS[order.productionStatus] || ORDER_STATUS["PENDING"];
        const paymentConfig =
          PAYMENT_STATUS[order.paymentStatus] || PAYMENT_STATUS["UNPAID"];
