...
export function ReceiptSection({
  ...
  setPriorityUrgent,
  discount,
  setDiscount, // NEW: Discount management
}) {
  ...
  const canCheckout =
    items.length > 0 && hasCustomerName && (isTempo || !isTunai || paid > 0);
  ...
  const renderSummary = () => {
    ...
    const sisaBayar = finalAmount - paid;
    ...
    return (
      <div className="total-summary">
        ...
        <div className="summary-row total">
          ...
          <span className="amount">{formatRupiah(finalAmount)}</span>
        </div>
                ...
                {finishings && finishings.length > 0 && (
                  <div className="item-finishings">
                    {finishings.map((f, idx) => (
                      <span key={idx} className="fin-pill">
                        + {f.name}
                      </span>
                    ))}
                  </div>
